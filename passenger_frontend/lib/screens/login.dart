        ...
        if (response.statusCode == 200) {
          ...
          if (responseData is List<dynamic>) {
            ...
            Map<String, dynamic>? user =
                AuthService.decodeJwtToken(accessToken);
            ...
            if (user != null) {
              ...
              await prefs.setString(
                  'nic', user['nic'] ?? ''); // Use empty string if null
              await prefs.setString('email', user['email']);
              await prefs.setString(
                  'dob', user['dob'].toString()); // Convert DateTime to String
              ...
              await prefs.setString(
                  'otp', user['otp'] ?? ''); // Use empty string if null
              await prefs.setString('accessToken',
                  user['accessToken'] ?? ''); // Use empty string if null
              await prefs.setString(
                  'otpGenerateTime',
                  user['otpGenerateTime']?.toString() ??
                      ''); // Convert DateTime to String, use empty string if null
              await prefs.setStringList(
                  'user_type', List<String>.from(user['userType']));
            } else {
